<div align="center">
  <img
    height="150"
    src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExeGZkMWt5eDE2dXhnMGtiNHNnOHB2ZnNyNXZmc3pnMGZwOGo3aW1qMSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/AfpXE9om1TtkcjpKhI/giphy.gif"
    style="pointer-events: none;"
  />
</div>

###

<div align="center">
<a href="https://linkedin.com/in/salemalnaqbi" target="_blank" rel="noopener noreferrer"><img src="https://img.shields.io/badge/LinkedIn-0077B5?logo=linkedin&logoColor=white&style=for-the-badge" height="25" /></a>&nbsp;&nbsp;
<a href="https://x.com/salem_alnaqbi" target="_blank" rel="noopener noreferrer"><img src="https://img.shields.io/badge/Twitter-1DA1F2?logo=twitter&logoColor=white&style=for-the-badge" height="25" /></a>&nbsp;&nbsp;
<a href="mailto:salem.alnaqbi@hotmail.com" target="_blank" rel="noopener noreferrer"><img src="https://img.shields.io/badge/Email-0078D4?logo=microsoft-outlook&logoColor=white&style=for-the-badge" height="25" /></a>
</div>


###

<h1 align="center">
  hey there
  <img
    src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExbmZ5cmwzdG5hMTd6NnlzYzU5ZWVkczQzMWozc3FkbWo5amF5OTdlbSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/w1OBpBd7kJqHrJnJ13/giphy.gif"
    width="70"
    style="pointer-events: none;"
  />
</h1>

###

<h3 align="left">About Me</h3>

<p align="left">
I’m Salem <br><br>
  
- 🔭 **AI & Energy Operations Engineer (Drilling)**  
  applying machine learning to real subsurface, drilling, and operational energy data to improve planning, performance, and decision-making

- 📚 **I’m currently learning advanced AI and machine learning**  
  with hands-on focus on deep learning, time-series modeling, and production-ready industrial AI systems

- ⚡ **In my free time, I build applied AI projects**  
  using real energy and subsurface datasets, turning engineering problems into deployable ML solutions
<br><br>
  
<h3 align="left">🚀 Projects</h3>


| # | Project | Description |
|---|---------|-------------|
| 1 | [Rock Classification using Deep Learning](https://github.com/SalemAlnaqbi/Rock-Classification) | CNN-based classification of carbonate rock types from images to support subsurface characterization. |
| 2 | [Water Quality pH Prediction Using Machine Learning](https://github.com/SalemAlnaqbi/Water-Quality-pH-Prediction-ML) | ML pipeline comparing Bayesian Ridge and LSTM for next-day pH prediction, achieving R²=0.83. |
| 3 | [Image Caption Generation with CNN-LSTM](https://github.com/SalemAlnaqbi/Image_Caption_Generation) | Generates image captions using ResNet-152 encoder and LSTM decoder, evaluated with BLEU-4. |
| 4 | [Insurance Fraud Detection](https://github.com/SalemAlnaqbi/Insurance-Fraud-Detection) | Detects fraudulent claims across a 28,836-record dataset using Random Forest and Logistic Regression, validated with ROC-AUC and nested cross-validation. |
| 5 | [Real-Time Geospatial Analysis of Climate Discourse](https://github.com/SalemAlnaqbi/Real-Time-Geospatial-Analysis) | Maps climate sentiment by country from live Telegram data using LDA topic modelling and VADER, visualised on an interactive Folium world map. |

<br><br>
<h3 align="left">🛠 Tech Toolbox</h3>

<div align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="40" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sqlite/sqlite-original.svg" height="40" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" height="40" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" height="40" />
  <img width="12" />
  <img src="https://cdn.simpleicons.org/pytorch/EE4C2C" height="40" />
  <img width="12" />
  <img src="https://cdn.simpleicons.org/tensorflow/FF6F00" height="40" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/matlab/matlab-original.svg" height="40" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jupyter/jupyter-original.svg" height="40" />
</div>

<br><br>

<h3 align="left">☁️ Platforms</h3>

<div align="left">
  <img src="https://skillicons.dev/icons?i=aws" height="40" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/googlecloud/googlecloud-original.svg" height="40" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" height="40" />
</div>

<br><br>


<h3 align="left">🔥 My Stats</h3>

[![GitHub Streak](http://github-readme-streak-stats.herokuapp.com?user=SalemAlnaqbi&theme=dark&background=000000)](https://git.io/streak-stats)

<br><br>

<div align="center"> <img height="150" src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExbnVjd3Rsb3drM2V5bzdpa2kwdms3bXEweTV2ZGJ1czZsamhpcDNqdyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/GKDUD40CLQHRumFw8i/giphy.gif" /> </div>

